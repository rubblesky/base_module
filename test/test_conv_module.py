...
class TestConvModule(TestModule):
    def __init__(self,data):
        ...
        module = torch.load("./pth/ConvModule.pth")
        module.eval()
        ...
        c_module = conv_module.build_conv_module(c_char_p("./bin/ConvModule.bin".encode('utf-8')))
    ...
    def diff(self):
        ...
        for i,input in enumerate(self.data):
            diff = []
            c_output[i] = cast(c_output[i],POINTER(c_float * py_output[i].shape[0] * py_output[i].shape[1] * py_output[i].shape[2] ))
            print("input: ",input)
            print("python output:",py_output[i])
            print("c output:", end='')
            for l1 in range(py_output[i].shape[1]):
                for l2 in range(py_output[i].shape[2]):
                    for l3 in range(py_output[i].shape[0]):
                        print(c_output[i].contents[l1][l2][l3],end=' ')
                        diff.append(float(abs(py_output[i][l3,l1,l2]-c_output[i].contents[l1][l2][l3])))
